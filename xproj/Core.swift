...
internal struct Core {
    ...
    mutating func processArgument() {
        do {
            ...
            let pbxFileReferenceString = try Container<PBXFileReference>.extractTargetFromCurrent(string: projectfile)
            let pbxFileReferenceCollection = try Parser().start(string: pbxFileReferenceString)
            let pbxFileReference = try Container<PBXFileReference>(data: pbxFileReferenceCollection)
            let pbxBuildFileString = try Container<PBXBuildFile>.extractTargetFromCurrent(string: projectfile)
            let pbxBuildFileCollection = try Parser().start(string: pbxBuildFileString)
            let pbxBuildFile = try Container<PBXBuildFile>(data: pbxBuildFileCollection)
            let pbxGroupString = try Container<PBXGroup>.extractTargetFromCurrent(string: projectfile)
            let pbxGroupCollection = try Parser().start(string: pbxGroupString)
            var pbxGroup = try Container<PBXGroup>(data: pbxGroupCollection)
            let pbxSourceBuildPhaseString = try Container<PBXSourcesBuildPhase>.extractTargetFromCurrent(string: projectfile)
            let pbxSourceBuildPhaseCollection = try Parser().start(string: pbxSourceBuildPhaseString)
            let pbxSourceBuildPhase = try Container<PBXSourcesBuildPhase>(data: pbxSourceBuildPhaseCollection)
//            let pbxProjectString = try Container<PBXProject>.extractTargetFromCurrent(string: projectfile)
//            let pbxProjectCollection = try Parser().start(string: pbxProjectString)
//            let pbxProject = try Container<PBXProject>(data: pbxProjectCollection)
            
            let mainGroup = try Parser().parseRootObjectUUID(string: projectfile)
            let allHashes = try Parser().parseAllHashValues(string: projectfile)
            
            PBXUUIDGenerator.shared.set(collection: allHashes)
            ...
            if let addableGroups = groupsMustBeAdded {
                do {
                    try addableGroups
                        ...
                        .forEach{ value in
                            let _ = try pbxGroup.findGroupByPath(parent: mainGroup, reversedPathArray: value, generateGroupIfNeeded: true, uuidGenerator: PBXUUIDGenerator.shared)
                    }
                } catch {
            ...
            let PBXGroupModified = try pbxGroup.generateProjectWithCurrent(string: projectfile, newline: true)
            let PBXSourceContainerModified = try pbxSourceBuildPhase.generateProjectWithCurrent(string: PBXGroupModified, newline: true)
            let PBXBuildFilesModified = try pbxBuildFile.generateProjectWithCurrent(string: PBXSourceContainerModified, newline: true)
            let PBXBuildFileReferenceModified = try pbxFileReference.generateProjectWithCurrent(string: PBXBuildFilesModified, newline: false)
